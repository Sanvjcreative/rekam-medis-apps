...
@section('page-title', 'Edit Pemeriksaan Laboratorium')
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0"><i class="bi bi-flask"></i> Form Edit Pemeriksaan Laboratorium</h5>
        <a href="{{ route('laboratorium.index') }}" class="btn btn-secondary btn-sm">
            <i class="bi bi-arrow-left"></i> Kembali
        </a>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('laboratorium.update', $id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('jenis_pemeriksaan') is-invalid @enderror" 
                        ...
                        <option value="Darah Lengkap" {{ old('jenis_pemeriksaan') == 'Darah Lengkap' ? 'selected' : '' }}>Darah Lengkap</option>
                        <option value="Urine" {{ old('jenis_pemeriksaan') == 'Urine' ? 'selected' : '' }}>Urine</option>
                        <option value="Feses" {{ old('jenis_pemeriksaan') == 'Feses' ? 'selected' : '' }}>Feses</option>
                        <option value="Gula Darah" {{ old('jenis_pemeriksaan') == 'Gula Darah' ? 'selected' : '' }}>Gula Darah</option>
                    </select>
