...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0"><i class="bi bi-person-plus"></i> Edit Data Pendaftaran</h5>
        <a href="{{ route('pendaftaran.index') }}" class="btn btn-secondary btn-sm">
            <i class="bi bi-arrow-left"></i> Kembali
        </a>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('pendaftaran.update', $id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('no_rm') is-invalid @enderror" 
                           id="no_rm" name="no_rm" value="{{ old('no_rm', 'RM000001') }}" 
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('nama_pasien') is-invalid @enderror" 
                           id="nama_pasien" name="nama_pasien" value="{{ old('nama_pasien', 'Pasien ' . $id) }}" 
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('poli') is-invalid @enderror" id="poli" name="poli" required>
                        ...
                        <option value="Umum" {{ old('poli', 'Umum') == 'Umum' ? 'selected' : '' }}>Poli Umum</option>
                        <option value="Anak" {{ old('poli', 'Anak') == 'Anak' ? 'selected' : '' }}>Poli Anak</option>
                        <option value="Gigi" {{ old('poli', 'Gigi') == 'Gigi' ? 'selected' : '' }}>Poli Gigi</option>
                        <option value="Mata" {{ old('poli', 'Mata') == 'Mata' ? 'selected' : '' }}>Poli Mata</option>
                        <option value="Kulit" {{ old('poli', 'Kulit') == 'Kulit' ? 'selected' : '' }}>Poli Kulit</option>
                    </select>
                    @error('poli')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('dokter') is-invalid @enderror" id="dokter" name="dokter" required>
                        ...
                        <option value="Dr. Ahmad" {{ old('dokter', 'Dr. Ahmad') == 'Dr. Ahmad' ? 'selected' : '' }}>Dr. Ahmad</option>
                        <option value="Dr. Siti" {{ old('dokter', 'Dr. Siti') == 'Dr. Siti' ? 'selected' : '' }}>Dr. Siti</option>
                        <option value="Dr. Budi" {{ old('dokter', 'Dr. Budi') == 'Dr. Budi' ? 'selected' : '' }}>Dr. Budi</option>
                        <option value="Dr. Dewi" {{ old('dokter', 'Dr. Dewi') == 'Dr. Dewi' ? 'selected' : '' }}>Dr. Dewi</option>
                    </select>
                    @error('dokter')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('keluhan') is-invalid @enderror" 
                          ...
                          placeholder="Masukkan keluhan pasien">{{ old('keluhan', 'Keluhan pasien') }}</textarea>
                @error('keluhan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
